...
def filter_region(points: np.ndarray, region: str) -> np.ndarray:
    ...
    if region == "all":
        ...
        mask = base & (r >= 10.0) & (r < 20.0)
    elif region == "far":
        mask = base & (r >= 20.0) & (r <= 35.0)
    elif region == "front":
...
def register_region(source_points: np.ndarray, target_points: np.ndarray) -> o3d.pipelines.registration.RegistrationResult | None:
...
def region_metrics(result, gt_rel):
    if result is None:
        return None
...
def main() -> None:
    ...
    for i in range(len(samples) - 1):
        ...
        for region in regions:
            src_region = filter_region(src_points, region)
            tgt_region = filter_region(tgt_points, region)
            result = register_region(src_region, tgt_region)
            metrics = region_metrics(result, gt_rel)
            pair_info["regions"][region] = {
                "num_source_points": int(src_region.shape[0]),
                "num_target_points": int(tgt_region.shape[0]),
                "metrics": metrics,
            }
    ...
    aggregate = {}
    for region in regions:
        valid = [p["regions"][region]["metrics"] for p in pair_results if p["regions"][region]["metrics"] is not None]
        if not valid:
            aggregate[region] = None
            continue
        aggregate[region] = {
    ...
    out_path = args.run_summary.parent / "pairwise_region_analysis.json"
